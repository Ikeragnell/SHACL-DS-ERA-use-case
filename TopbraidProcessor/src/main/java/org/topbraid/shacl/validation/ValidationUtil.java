...
import org.topbraid.shacl.ds.ShapesDatasetEngine;
...
public class ValidationUtil {
    ...
    /**
     * Validates a given data Dataset against all shapes graphs declared in a shapes Dataset
     * (SHACL-DS / SHACL for Datasets).
     * <p>
     * Each named graph in the shapes dataset is treated as an independent shapes graph.
     * The target data graphs for each shapes graph are resolved from {@code shds:targetGraph},
     * {@code shds:targetGraphPattern}, and {@code shds:targetGraphCombination} declarations.
     * For each (shapes graph, focus graph) pair a SHACL-DS dataset view is constructed and
     * validated using the standard {@link ValidationEngine}.
     * <p>
     * Every {@code sh:ValidationResult} in the merged report is annotated with
     * {@code shds:sourceShapesGraph} and {@code shds:focusGraph}.
     *
     * @param shapesDataset the shapes dataset (named graphs are individual shapes graphs)
     * @param dataDataset   the data dataset to validate
     * @return a merged {@code sh:ValidationReport} resource; its model contains all results
     * @see ShapesDatasetEngine
     */
    public static Resource validateDataset(Dataset shapesDataset, Dataset dataDataset) {
        return validateDataset(shapesDataset, dataDataset, new DatasetValidationEngineConfiguration());
    }


    /**
     * Validates a given data Dataset against all shapes graphs declared in a shapes Dataset
     * (SHACL-DS / SHACL for Datasets), using the supplied configuration.
     *
     * @param shapesDataset the shapes dataset (named graphs are individual shapes graphs)
     * @param dataDataset   the data dataset to validate
     * @param configuration configuration forwarded to each per-graph {@link ValidationEngine}
     * @return a merged {@code sh:ValidationReport} resource; its model contains all results
     * @see ShapesDatasetEngine
     */
    public static Resource validateDataset(Dataset shapesDataset, Dataset dataDataset,
            DatasetValidationEngineConfiguration configuration) {
        return ShapesDatasetEngine.validate(shapesDataset, dataDataset, configuration);
    }
}
